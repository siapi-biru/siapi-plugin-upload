    ...
    Mutation: {
      ...
      upload: {
        ...
        resolver: async (obj, { file: upload, info, ...fields }) => {
          ...
          const uploadedFiles = await siapi.plugins.upload.services.upload.uploadFileAndPersist(
            file
          );
      ...
      multipleUpload: {
        ...
        resolver: async (obj, { files: uploads, ...fields }) => {
          ...
          const uploadService = siapi.plugins.upload.services.upload;
      ...
      updateFileInfo: {
        ...
        resolver: async (obj, { id, info }) => {
          return await siapi.plugins.upload.services.upload.updateFileInfo(id, info);
        },
      },
      deleteFile: {
        ...
        resolver: async (obj, options, { context }) => {
          const file = await siapi.plugins.upload.services.upload.fetch({ id: context.params.id });
          if (file) {
            const fileResult = await siapi.plugins.upload.services.upload.remove(file);
