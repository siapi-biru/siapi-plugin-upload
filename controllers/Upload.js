...
module.exports = {
  ...
  async upload(ctx) {
    const isUploadDisabled = _.get(siapi.plugins, 'upload.config.enabled', true) === false;
  ...
  async search(ctx) {
    const { id } = ctx.params;
    const model = siapi.getModel('file', 'upload');
    const entries = await siapi.query('file', 'upload').custom(searchQueries)({
      id,
    });
